...
class EventEmitter {
	...
	static async emit(event) {
		...
		try {
			...
			const headers = event.client ? { 'janis-client': event.client } : {};

			const response = await ms.post(this._eventsService, this._eventsNamespace, this._eventsMethod, { ...event, service: this._serviceName }, headers);
