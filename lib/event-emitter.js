...
const { JANIS_SERVICE_NAME } = process.env;
...
class EventEmitter {
	...
	static async emit(event) {
		...
		try {
			...
			await ms.post(this._eventsService, this._eventsNamespace, this._eventsMethod, { ...event, service: this._serviceName });

		} catch(err) {
			throw new EventEmitterError(err.message, EventEmitterError.codes.MS_CALL_ERROR);
		}

		// En caso de que MicroserviceCall devuelva un 200
		return true;
	}
